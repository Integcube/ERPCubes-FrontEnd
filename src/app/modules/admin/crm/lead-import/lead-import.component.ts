...
export class LeadImportComponent implements OnInit {
  ...
  applyColumnMapping() {
    ...
    this.leadImportService.getProduct().subscribe((products: Product[]) => {
      this.leadImportService.getIndustries().subscribe((industries: Industry[]) => {
          ...
          if (excelColumn !== 'First Name' && excelColumn !== 'Last Name') {
            ...
            if (row.hasOwnProperty(excelColumn)) {
              if (leadColumn.toLowerCase() === 'producttitle' || leadColumn.toLowerCase() === 'industrytitle') {
                ...
                leadData[idField] = matchingItem ? matchingItem[idField] : '';
              } else {
                // Set the value to a string (wrapped in double quotes) if the column is not present in the current row
                leadData[leadColumn.toLowerCase()] = typeof row[excelColumn] === 'number'
                  ? `"${row[excelColumn]}"`
                  : row[excelColumn] || '';
              }
            } else {
              // Set the value to an empty string if the column is not present in the current row
              leadData[leadColumn.toLowerCase()] = '';
            }
          }
        });
      ...
      const c = mappedLeads.map((lead) => {
        ...
        Object.keys(c).forEach((field) => {
          ...
          if (c[field] !== null && c[field] !== undefined) {
            // Convert the field to string and wrap it in double quotes
  ...
  moveStepperToNextStep(): void {
    if (this.stepper.selectedIndex < this.stepper.steps.length - 1) {
      if (this.stepper.selectedIndex === 0 && !this.isSelected) {
        ...
        if (this.fileUploaded && this.isFileValid) {
